...
Real mass, temp, f, rhocrit, omega, bz, mu, amp, tilt, tiltrad;
    ...
    for (int k=pmb->ks; k<=pmb->ke; ++k) {
      for (int j=pmb->js; j<=pmb->je; ++j) {
        for (int i=pmb->is; i<=pmb->ie; ++i) {
          Real ke = 0.5 / cons(IDN,k,j,i)
                  * (SQR(cons(IM1,k,j,i)) + SQR(cons(IM2,k,j,i)) + SQR(cons(IM3,k,j,i)));
          Real me = 0.5*(SQR(cons(IBX1,k,j,i)) + SQR(cons(IBX2,k,j,i)) + SQR(cons(IBX3,k,j,i)));
          Real te = igm1 * cons(IDN,k,j,i)
                  * std::sqrt(1.0+std::pow(cons(IDN,k,j,i)/rhocrit, 2.0*gm1));
          cons(IEN,k,j,i) = te + ke + me;
        }
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  tilt = pin->GetOrAddReal("problem", "tilt", 0.0); // angle
  tiltrad = tilt/180.0*PI;
  ...
  if (Globals::my_rank == 0 && ncycle == 0) {
    std::cout << std::endl
      ...
      << "Magnetic field      : " << gauss     << " \t[Gauss]" << std::endl
      << "Tilt Angle          : " << tilt      << " \t[Degrees]" << std::endl << std::endl
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  Real cost = std::cos(tiltrad), sint = std::sin(tiltrad);
    ...
    for (int j=js; j<=je; ++j) {
      ...
      for (int i=is; i<=ie; ++i) {
        ...
        phydro->u(IDN,k,j,i) = f * BEProfile(r)
               * (1.0+amp*SQR(r)/SQR(rc)*cos(2.0*std::atan2(y,x*cost-z*sint)));
        if (r < rc) {
          phydro->u(IM1,k,j,i) =  phydro->u(IDN,k,j,i)*omega*y*cost;
          phydro->u(IM2,k,j,i) = -phydro->u(IDN,k,j,i)*omega*(x*cost - z*sint);
          phydro->u(IM3,k,j,i) = -phydro->u(IDN,k,j,i)*omega*y*sint;
        } else {
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) {
        for (int i=is; i<=ie+1; ++i) {
          pfield->b.x1f(k,j,i) = bz*std::sin(tiltrad);
        }
      }
    }
    for (int k=ks; k<=ke; ++k) {
    ...
    for (int k=ks; k<=ke+1; ++k) {
      for (int j=js; j<=je; ++j) {
        for (int i=is; i<=ie; ++i) {
          pfield->b.x3f(k,j,i) = bz*std::cos(tiltrad);
        }
      }
    }
    if (NON_BAROTROPIC_EOS) {
    ...
    for (int k=ks; k<=ke; k++) {
      for (int j=js; j<=je; j++) {
        for (int i=is; i<=ie; i++) {
          phydro->u(IBX1,k,j,i) = bz*std::sin(tiltrad);
          phydro->u(IBX2,k,j,i) = 0.0;
          phydro->u(IBX3,k,j,i) = bz*std::cos(tiltrad);
