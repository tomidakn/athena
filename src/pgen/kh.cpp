...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  if (iprob == 4) {
    ...
    if (MAGNETIC_FIELDS_ENABLED) {
      ...
      for (int k=ks; k<=ke; k++) {
        for (int j=js; j<=je; j++) {
          for (int i=is; i<=ie; i++) {
            phydro->u(IBX1,k,j,i) = b0*std::tanh((std::abs(pcoord->x2v(j)) - 0.5)/a);
            phydro->u(IBX2,k,j,i) = 0.0;
            phydro->u(IBX3,k,j,i) = 0.0;
            phydro->u(IPS,k,j,i) = 0.0;
            if (NON_BAROTROPIC_EOS)
  ...
  if (iprob == 5) { // Lecoanet but with uniform B
    // Read/set problem parameters
    Real amp = pin->GetReal("problem","amp");
    // unstratified problem is the default
    Real drho_rho0 = pin->GetOrAddReal("problem", "drho_rho0", 0.0);
  ...
  //--- iprob=6. Uniform stream with density ratio "drat" located in region -1/4<y<1/4
...
//      b0 = b0/std::sqrt(4.0*(PI));
  ...
  if (iprob == 9) { // Lecoanet but with only one Gaussian B
    // Read/set problem parameters
    Real amp = pin->GetReal("problem","amp");
    // unstratified problem is the default
    Real drho_rho0 = pin->GetOrAddReal("problem", "drho_rho0", 0.0);
    ...
    if (MAGNETIC_FIELDS_ENABLED) {
      ...
      for (int k=ks; k<=ke; k++) {
        for (int j=js; j<=je; j++) {
          for (int i=is; i<=ie+1; i++) {
            pfield->b.x1f(k,j,i) = b0*std::exp(-(SQR(pcoord->x2v(j)-z1))/(sigma2*sigma2));
          }
        }
      }
      for (int k=ks; k<=ke; k++) {
      ...
      for (int k=ks; k<=ke; k++) {
        for (int j=js; j<=je; j++) {
          for (int i=is; i<=ie; i++) {
            phydro->u(IBX1,k,j,i) = b0*std::exp(-(SQR(pcoord->x2v(j)-z1))/(sigma2*sigma2));
            phydro->u(IBX2,k,j,i) = 0.0;
            phydro->u(IBX3,k,j,i) = 0.0;
            phydro->u(IPS,k,j,i) = 0.0;
            if (NON_BAROTROPIC_EOS)
