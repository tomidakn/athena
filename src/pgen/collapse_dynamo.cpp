  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    ...
    if (NON_BAROTROPIC_EOS) {
      ...
      for (int k = pmb->ks-NGHOST; k<=pmb->ke+NGHOST; ++k) {
        for (int j = pmb->js-NGHOST; j<=pmb->je+NGHOST; ++j) {
          for (int i = pmb->is-NGHOST; i<=pmb->ie+NGHOST; ++i) {
            Real v = std::sqrt(gamma * pmb->phydro->w(IPR,k,j,i)
                              / pmb->phydro->w(IDN,k,j,i))+
                   + std::sqrt((SQR(pmb->phydro->w(IBX1,k,j,i))
                              + SQR(pmb->phydro->w(IBX2,k,j,i))
                              + SQR(pmb->phydro->w(IBX3,k,j,i)))
                              / pmb->phydro->w(IDN,k,j,i));
            Real nj = v / std::sqrt(pmb->phydro->w(IDN,k,j,i));
            njmin = std::min(njmin, nj);
          }
      ...
      for (int k = pmb->ks-NGHOST; k<=pmb->ke+NGHOST; ++k) {
        for (int j = pmb->js-NGHOST; j<=pmb->je+NGHOST; ++j) {
          for (int i = pmb->is-NGHOST; i<=pmb->ie+NGHOST; ++i) {
            Real v = cs + std::sqrt((SQR(pmb->phydro->w(IBX1,k,j,i))
                                   + SQR(pmb->phydro->w(IBX2,k,j,i))
                                   + SQR(pmb->phydro->w(IBX3,k,j,i)))
                                   / pmb->phydro->w(IDN,k,j,i));
            Real nj = v / std::sqrt(pmb->phydro->w(IDN,k,j,i));
...
void Cooling(MeshBlock *pmb, const Real time, const Real dt,
  ...
  constexpr Real gml = 1.1 - 1.0, gmh = 2.0 - 1.0;
      ...
      for (int j=pmb->js; j<=pmb->je; ++j) {
        for (int i=pmb->is; i<=pmb->ie; ++i) {
          ...
          Real te = igm1 * cons(IDN,k,j,i)
                  * std::sqrt(std::pow(std::max(1.0, cons(IDN,k,j,i)), 2.0*gml)
                             +std::pow(cons(IDN,k,j,i)/rhocrit, 2.0*gmh));
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  for (int k=ks; k<=ke; ++k) {
    ...
    for (int j=js; j<=je; ++j) {
      ...
      for (int i=is; i<=ie; ++i) {
        ...
        if (r < rc) {
          phydro->u(IM1,k,j,i) = -phydro->u(IDN,k,j,i)*omega*y*(1.0+amp*std::cos(2.0*phi));
          phydro->u(IM2,k,j,i) =  phydro->u(IDN,k,j,i)*omega*x*(1.0+amp*std::cos(2.0*phi));
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    ...
    for (int k=ks; k<=ke; k++) {
      for (int j=js; j<=je; j++) {
        for (int i=is; i<=ie; i++) {
          phydro->u(IBX1,k,j,i) = 0.0;
          phydro->u(IBX2,k,j,i) = 0.0;
          phydro->u(IBX3,k,j,i) = bz;
