...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  } else { // CC_MAGNETIC_FIELDS
    for (int k=ks; k<=ke; k++) {
      for (int j=js; j<=je; j++) {
        for (int i=is; i<=ie; i++) {
          phydro->u(IBX1,k,j,i) = -B0*std::sin(TWO_PI*pcoord->x2v(j));
          phydro->u(IBX2,k,j,i) =  B0*std::sin(4.0*PI*pcoord->x1v(i));
          phydro->u(IBX3,k,j,i) = 0.0;
          phydro->u(IPS,k,j,i) = 0.0;
          phydro->u(IEN,k,j,i) = p0/gm1
              + 0.5*(SQR(phydro->u(IBX1,k,j,i))
                   + SQR(phydro->u(IBX2,k,j,i))
