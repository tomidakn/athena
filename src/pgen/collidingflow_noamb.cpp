...
void ConductionCoeff(HydroDiffusion *phdif, MeshBlock *pmb,
     const AthenaArray<Real> &prim, const AthenaArray<Real> &bcc,
...
void TwoPhaseInnerX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
                     FaceField &b, Real time, Real dt,
                     int is, int ie, int js, int je, int ks, int ke, int ngh);
void TwoPhaseOuterX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
                     FaceField &b, Real time, Real dt,
                     int is, int ie, int js, int je, int ks, int ke, int ngh);

inline Real mytanh(Real x){
  Real ex = std::exp(2.0*x);
  return (ex-1.0)/(ex+1.0);
}

inline Real molecular_weight(Real num){
  return 0.53*mytanh( (std::log10(num) - 1.3)/0.9 ) + 1.8;
}
...
if( pin->GetOrAddReal("problem","nu_iso",0.0) == 0.0 ){
  ...
  primbou0.NewAthenaArray(iend+1,iend+1,iend+1); // density
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        ...
        if ( xx < xbou(0)    ) xx += Lbou;
        if ( xx > xbou(iend) ) xx -= Lbou;
  ...
  P0 = rho0*temeq/(0.53*(mytanh( (std::log10(n0) - 1.3)/0.9 ) + 1.0) + 1.27);
  if( Globals::my_rank == 0 ){
    printf("T0 = %.5e\n",temeq*Tunit);
  }
...
#pragma clang loop vectorize(assume_safety)
      for (int i=is; i<=ie; ++i) {
        ...
        Real ln=std::max((std::min(std::log10(numH),logn0_max)-logn0_min)/dlogn0,0.0);
        int in=(int)ln;
        Real wn=ln-in;
        Real mu = mol(in)*(1.0 - wn) + mol(in+1)*wn;
        Real tem0 = mu*Tunit*prim(IPR,k,j,i)/prim(IDN,k,j,i);

        Real lt=std::max((std::min(std::log10(tem0),logT0_max)-logT0_min)/dlogT0,0.0);
        int it=(int)lt;
        Real wt=lt-it;
...
//----------------------------------------------------------------------------------------
void DefineSimulationUnits(void) {
