...
//! \brief Converts conserved into primitive variables in adiabatic cc-MHD
...
#pragma omp simd
      for (int i=il; i<=iu; ++i) {
        ...
        const Real& u_bx = cons(IBX1,k,j,i);
        const Real& u_by = cons(IBX2,k,j,i);
        const Real& u_bz = cons(IBX3,k,j,i);
        const Real& u_ps = cons(IPS,k,j,i);
        ...
        Real& w_bx = prim(IBX1,k,j,i);
        Real& w_by = prim(IBX2,k,j,i);
        Real& w_bz = prim(IBX3,k,j,i);
        Real& w_ps = prim(IPS,k,j,i);
...
#pragma omp simd
      for (int i=il; i<=iu; ++i) {
        ...
        Real& u_e  = cons(IEN,k,j,i);
        Real& u_bx = cons(IBX1,k,j,i);
        Real& u_by = cons(IBX2,k,j,i);
        Real& u_bz = cons(IBX3,k,j,i);
        ...
        const Real& w_bx = prim(IBX1,k,j,i);
        const Real& w_by = prim(IBX2,k,j,i);
        const Real& w_bz = prim(IBX3,k,j,i);
        const Real& w_ps = prim(IPS,k,j,i);
...
Real EquationOfState::FastMagnetosonicSpeed(const Real prim[(NWAVE)], const Real bx) {
  ...
  Real vaxsq = bx*bx;
  Real ct2 = (prim[IBX2]*prim[IBX2] + prim[IBX3]*prim[IBX3]);
