...
//========================================================================================
//! \file hlld_ccmhd.cpp
//! \brief HLLD Riemann solver for adiabatic MHD with divB cleaning.
//!
//! REFERENCES:
...
void Hydro::RiemannSolver(const int k, const int j, const int il, const int iu,
  ...
  int ibx = ivx - IVX + IEN + 1;
  int iby = IBX1 + ((ivx-IVX)+1)%3;
  int ibz = IBX1 + ((ivx-IVX)+2)%3;
...
#pragma omp simd simdlen(SIMD_WIDTH) private(wli,wri,spd,flxi)
  for (int i=il; i<=iu; ++i) {
    ...
    Real bxi = 0.5*((wli[IBX1] + wri[IBX1]) - (wri[IPS] - wli[IPS])/ch_);
    ...
    Real pbl = 0.5*(bxsq + (SQR(wli[IBX2]) + SQR(wli[IBX3])));  // magnetic pressure (l/r)
    Real pbr = 0.5*(bxsq + (SQR(wri[IBX2]) + SQR(wri[IBX3])));
    ...
    if (spd[0] >= 0.0) {
      ...
      flxi[IBX2] = fl.by + ulst.by;
      flxi[IBX3] = fl.bz + ulst.bz;
    } else if (spd[3] <= 0.0) {
      // return Fr*
      ...
      flxi[IBX2] = fr.by + urst.by;
      flxi[IBX3] = fr.bz + urst.bz;
    } else if (spd[2] >= 0.0) {
      // return Fl**
      ...
      flxi[IBX2] = fl.by + ulst.by + uldst.by;
      flxi[IBX3] = fl.bz + ulst.bz + uldst.bz;
    } else {
      // return Fr**
      ...
      flxi[IBX2] = fr.by + urst.by + urdst.by;
      flxi[IBX3] = fr.bz + urst.bz + urdst.bz;
    }
